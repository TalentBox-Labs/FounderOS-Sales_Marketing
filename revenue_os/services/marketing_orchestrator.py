...
def run_marketing_cycle(query: str = "", business_context: str = "") -> dict[str, Any]:
    ...
    research = marketing_research.run_market_research(seed_topic)
    ...
    persona_result = marketing_strategy.update_customer_persona()
    ...
    strategy = marketing_strategy.build_content_strategy(ctx)
    report["stages"]["content_strategy"] = {"monthly_theme": strategy.get("monthly_theme"), "weeks": strategy.get("weeks", [])}

    seo = marketing_strategy.build_seo_strategy(ctx)
    ...
    written = marketing_content.write_content("blog post", first_topic, ctx, publish=True)
    report["stages"]["content_writer"] = {"article_id": written.get("article_id"), "title": written.get("title")}

    video = marketing_content.plan_video(first_topic, ctx)
    report["stages"]["video_strategy"] = {"hook": video.get("hook"), "talking_points": video.get("talking_points")}

    brief = marketing_content.create_design_brief("social graphic", first_topic, ctx)
    report["stages"]["creative_design"] = {"concept": brief.get("concept"), "headline": brief.get("headline")}

    social = marketing_content.draft_social_posts(first_topic, ["linkedin", "x"], ctx)
    report["stages"]["social_media"] = {"approval_id": social.get("approval_id")}

    email = marketing_campaigns.draft_email_campaign("newsletter", ctx)
    ...
    campaign = marketing_campaigns.plan_campaign(
        ...
        channels=["seo", "blog", "linkedin", "social", "email"], context=ctx,
    )
    ...
    analytics = marketing_analytics.generate_analytics_report()
