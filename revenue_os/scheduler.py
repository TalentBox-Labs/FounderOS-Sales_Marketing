...
def job_scan_booking_eligibility() -> dict[str, Any]:
    """Propose governed bookings per authorized org (human approval still required).

    ACP-2: proposal is orchestrated AUTONOMOUS; calendar write remains HUMAN_REQUIRED
    via ApprovalRequest(book_meeting). ACP-3: pause/kill gate before discovery.
    ACP-4: each contact propose runs under orchestrate_claimed.
    """
    from revenue_os.services.acp2_work_contract import WORK_BOOKING_PROPOSE, WorkState
    from revenue_os.services.acp3_durable_runtime import gate_new_mutating_work
    from revenue_os.services.acp4_production_runtime import orchestrate_claimed
    from revenue_os.services.booking_eligibility import scan_eligible_bookings
    from revenue_os.services.revenue_orchestration_service import run_booking_proposal_scheduled
    ...
    db = SessionLocal()
    try:
        orgs, blocked = _resolve_orgs_or_block(db, action_type="booking_scan_blocked")
        if blocked is not None:
            return blocked
        ...
        candidates_total = 0
        for organization_id in orgs or []:
            candidates = scan_eligible_bookings(db, organization_id=organization_id)
            candidates_total += len(candidates)
            for item in candidates:
                if str(item.get("organization_id")) != str(organization_id):
                    skipped += 1
                    log_autonomous_blocked(
                        actor=ACTOR,
                        action_type="booking_proposal_blocked",
                        reason="tenant_entity_mismatch",
                        organization_id=organization_id,
                        target_type="contact",
                        target_id=item.get("contact_id"),
                ...
                def _exec(work, it=item, oid=organization_id):  # noqa: ANN001
                    result = run_booking_proposal_scheduled(
                        db, oid, it["contact_id"]
                    )
                    if result.get("ok"):
                        log_agent_action(
                            actor=ACTOR,
                            action_type="booking_proposal_scheduled",
                            target_type="contact",
                            target_id=it["contact_id"],
                            organization_id=oid,
                            detail={
                ...
                work = orchestrate_claimed(
                    ...
                    target_type="contact",
                    target_id=item["contact_id"],
                    logical_key=str(item.get("idempotency_key") or "book"),
                )
                if work.state in (WorkState.SUCCEEDED, WorkState.WAITING_HUMAN):
                    proposed += 1
                else:
...
def initialize_heartbeat() -> HeartbeatScheduler:
    ...
    scheduler.register(
        "scan_booking_eligibility", job_scan_booking_eligibility,
        _env_int("HEARTBEAT_BOOKING_SCAN_SEC", 3600),
    )
    scheduler.register(
