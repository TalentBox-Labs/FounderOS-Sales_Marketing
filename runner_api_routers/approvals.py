...
def list_approvals(
    ...
    return {"ok": True, "count": len(requests),
            "pending": pending_count(), "requests": requests}
...
def approve_request(
    ...
    try:
        result = decide(request_id, approve=True, decided_by=req.decided_by, note=req.note)
...
def reject_request(
    ...
    try:
        result = decide(request_id, approve=False, decided_by=req.decided_by, note=req.note)
    except ValueError as e:
        raise HTTPException(status_code=409 if "already" in str(e) else 404, detail=str(e))
    return {"ok": True, "request": result}
